...
class Fraction;	//ѕросто объ€вление класса
Fraction operator*(Fraction left, Fraction right);
Fraction operator/(Fraction left, Fraction right);

class Fraction	//ќбъ€вление и описание класса
{
...
Fraction operator*(Fraction left, Fraction right)
{
	...
	//result.to_proper();
	//return result;
	return Fraction	//явно вызываем конструктор, и создаем временный безым€нный объект,
	(//этот объект сразу же возвращаетс€ на место вызова.
		left.get_numerator()*right.get_numerator(),
		left.get_denominator()*right.get_denominator()
	).to_proper();
}
...
//		l-value = r-value;
	...
	char e = 515;	//Conversion from int to char. Truncation (”сечение, урезание)
